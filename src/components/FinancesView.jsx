...
export default function FinancesView() {
  ...
  const fetchFinances = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${GET_URL}?t=${new Date().getTime()}`, {
        method: 'GET',
      ...
      if (Array.isArray(data)) {
        normalized = Array.isArray(data[0]) ? data[0] : data;
      } else if (data && typeof data === 'object') {
        normalized = data.data || data.rows || data.items || (data.id ? [data] : []);
      }

      setFinances(Array.isArray(normalized) ? normalized : []);
    } catch (error) {
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        amount_usd: paymentForm.amount_usd,
        payment_method: paymentForm.payment_method,
        reference_number: paymentForm.payment_method === 'Efectivo USD' ? '' : paymentForm.reference_number,
        exchange_rate_bcv: paymentForm.exchange_rate_bcv || null,
        amount_bs: paymentForm.amount_bs || null,
  ...
  const selectedBalance = selectedRecord ? getBalance(selectedRecord) : 0;
          ...
          <table className="w-full min-w-[980px] text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredFinances.length === 0 ? (
                ...
                filteredFinances.map((row, idx) => {
                  const status = getStatus(row);
                  const balance = getBalance(row);
                  const paid = getPaid(row);
                  const agreed = getAgreed(row);

                  return (
                    <tr key={getLeadTreatmentId(row) || idx} className="hover:bg-slate-50/70">
                      <td className="px-4 py-3 align-top">
                        <div className="font-semibold text-slate-800">{getPatientName(row)}</div>
                        <div className="text-xs text-slate-500 mt-0.5 font-mono">CI: {getCedula(row)}</div>
                      </td>
                      <td className="px-4 py-3 align-top text-slate-700">{getTreatment(row)}</td>
                      <td className="px-4 py-3 align-top font-semibold text-slate-800">{formatUsd(agreed)}</td>
                      <td className="px-4 py-3 align-top text-green-700 font-semibold">{formatUsd(paid)}</td>
                      <td className="px-4 py-3 align-top text-rose-700 font-semibold">{formatUsd(balance)}</td>
                      <td className="px-4 py-3 align-top">
