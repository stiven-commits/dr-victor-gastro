...
const VZLA_STATES = ['Amazonas', 'Anzo?tegui', 'Apure', 'Aragua', 'Barinas', 'Bol?var', 'Carabobo', 'Cojedes', 'Delta Amacuro', 'Distrito Capital', 'Falc?n', 'Gu?rico', 'La Guaira', 'Lara', 'M?rida', 'Miranda', 'Monagas', 'Nueva Esparta', 'Portuguesa', 'Sucre', 'T?chira', 'Trujillo', 'Yaracuy', 'Zulia'];
...
export default function Dashboard() {
  ...
  const hasEditPermission = currentUser.role === 'admin' || (currentUser.permissions && currentUser.permissions[activeTab] === 'edit');
...
// Efecto para actualizar el t?tulo de la pesta?a del navegador
  useEffect(() => {
    let title = 'CRM Dr. V?ctor';
    if (activeTab === 'leads') title = 'CRM Dr. V?ctor - Leads';
    if (activeTab === 'patients') title = 'CRM Dr. V?ctor - Pacientes';
    if (activeTab === 'finances') title = 'CRM Dr. V?ctor - Finanzas';
    if (activeTab === 'informes') title = 'CRM Dr. V?ctor - Informes M?dicos';
    if (activeTab === 'audit') title = 'CRM Dr. V?ctor - Auditor?a';
    if (activeTab === 'inventory') title = 'CRM Dr. V?ctor - Inventario';
  ...
  const handleCreateManual = async (e) => {
    ...
    // MAGIA DE FIRMAS: Generar consentimiento autom?tico si es paciente
    ...
    if (newManualData.is_patient) {
      ...
      automaticConsentLog = ''; // Empezar vac?o
      ...
      if (needsAnesthesia) {
        automaticConsentLog += `\n===============================\nFIRMA DE ANESTESIA (MANUAL)\n===============================\nRegistrado por: Recepci?n / Dashboard\nPaciente: ${patientIdentifier}\nFecha y Hora: ${consentDate}\nDeclaraci?n: Se asume consentimiento f?sico/verbal para sedaci?n verificado.\n`;
      }
    }
    ...
    // Limpiar tambi?n los datos del representante al cerrar
    ...
    try {
      ...
      // 2. Descargar inmediatamente la base de datos para obtener el ID real que se le asign?
  ...
  const handleSaveEdit = (e) => {
    ...
    // MAGIA 2: Comparamos contra lo que de verdad est? en finanzas para ver si hay que crear deudas
    ...
    let removedTreatments = oldT; // Lo que sobr? fue eliminado
  ...
  const handleDeleteLead = async () => {
    ...
    // Actualizaci?n optimista: lo quitamos de la pantalla al instante
  ...
  return (
    <div className="flex min-h-screen font-sans bg-gray-50">
      ...
      {/* min-w-0 es la clave para evitar que las tablas desborden el ancho del m?vil */}
        ...
        <header className="flex items-center justify-between mb-6 md:mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl md:text-3xl font-bold text-slate-800 truncate">
              {activeTab === 'leads' ? 'Gestión de Leads' : activeTab === 'patients' ? 'Historial de Pacientes' : activeTab === 'agenda' ? 'Agenda M?dica' : activeTab === 'finances' ? 'Finanzas / Pagos' : activeTab === 'informes' ? 'Informes Médicos' : activeTab === 'inventory' ? 'Inventario Balones' : activeTab === 'users' ? 'Gestión de Usuarios' : 'Registro de Auditoría'}
            </h1>
          </div>
          {hasEditPermission && activeTab !== 'audit' && activeTab !== 'agenda' && activeTab !== 'inventory' && activeTab !== 'users' && (
            <button onClick={() => setAddModalOpen(true)} className="bg-[#0056b3] text-white px-3 md:px-5 py-2 md:py-2.5 rounded-xl font-bold hover:bg-blue-700 transition flex items-center gap-1 md:gap-2 shadow-sm text-sm md:text-base whitespace-nowrap">
              ...
              <span className="hidden md:inline">A?adir Registro</span><span className="md:hidden">A?adir</span>
            </button>
          )}
        </header>
        ...
        <div className="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl flex flex-col">
          {loading && activeTab !== 'audit' && activeTab !== 'inventory' ? (
            ...
            <div className="overflow-x-auto">
              ...
              {activeTab === 'informes' ? (
                ...
                <div>
                  <div className="flex justify-end items-center px-6 py-4 border-b border-gray-100 bg-slate-50"><button onClick={fetchAuditLogs} className="text-xs text-slate-500 hover:underline">Actualizar</button></div>
                  <table className="w-full text-sm text-left min-w-[800px]">
                    <thead className="bg-slate-800 text-white"><tr><th className="px-6 py-4">Fecha</th><th className="px-6 py-4">Usuario</th><th className="px-6 py-4">Paciente</th><th className="px-6 py-4">Acci?n</th></tr></thead>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {paginatedList.map((lead) => (
                        <Fragment key={lead.id}>
                          ...
                          {/* Fila expandible para M?viles (Leads) */}
                  ...
                  <p className="px-6 py-2 text-xs text-slate-400 bg-purple-50 border-b border-purple-100">?? Doble clic para editar datos o tratamientos.</p>
                    ...
                    <thead className="bg-purple-50 text-purple-900">
                      <tr>
                        ...
                        <th className="px-6 py-4 font-semibold border-l border-purple-100 hidden md:table-cell">Evoluci?n</th>
                        ...
                        <Fragment key={lead.id}>
                          <tr onDoubleClick={() => handleRowDoubleClick(lead)} className="transition hover:bg-purple-50/40 cursor-pointer group">
                            ...
                            <td className="px-6 py-4 align-top border-l border-purple-50 hidden md:table-cell">
                              ...
                              <div className="text-xs text-slate-600 mt-1"><span className="text-slate-400 font-mono text-[10px]">Edad:</span> {lead.edad ? `${lead.edad} a?os` : 'N/A'}</div>
                          ...
                          {/* Fila expandible para M?viles (Pacientes) */}
                          {expandedRowId === lead.id && (
                            <tr className="md:hidden bg-purple-50/60 border-b border-purple-100">
                              <td colSpan="2" className="px-4 py-5 shadow-inner">
                                <div className="flex flex-col gap-4">
                                  ...
                                  {/* Tratamientos */}
                                  <div>
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Tratamientos M?dicos</span>
                                    <div className="flex flex-wrap gap-1">
                                  ...
                                  {/* Medidas y Evoluci?n en cuadr?cula */}
          ...
          {(activeTab === 'leads' || activeTab === 'patients') && totalPages > 1 && (
            <div className="flex justify-between items-center px-6 py-4 bg-white border-t border-gray-100">
              ...
              <span className="text-sm font-medium text-slate-500">P?gina {currentPage} de {totalPages}</span>
