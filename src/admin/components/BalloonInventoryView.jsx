...
export default function BalloonInventoryView() {
  ...
  const fetchInventory = async () => {
    ...
    try {
      ...
      // ELIMINAR DUPLICADOS EN EL FRONTEND (Por si n8n los envía)
  ...
  return (
    <div className="p-6 space-y-8 animate-fade-in">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsModalOpen(true)} className="bg-[#0056b3] text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-blue-700 transition shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {BALLOON_BRANDS.map((brand) => {
          ...
          // Usamos el cálculo de SQL (current_stock) o 0 si no existe
          ...
          return (
            <div key={brand.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-5">
              ...
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{brand.name} ({brand.duration})</p>
                <p className="text-3xl font-black text-slate-800">{stock} <span className="text-sm font-medium text-slate-400">unid.</span></p>
              </div>
            </div>
          );
        })}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-50 bg-slate-50/50">
          <h3 className="font-bold text-slate-700 flex items-center gap-2"><History size={18} /> Historial de Movimientos</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td colSpan="6" className="text-center py-10 text-slate-400">Cargando movimientos...</td></tr>
              ) : history.length === 0 ? (
                <tr><td colSpan="6" className="px-6 py-10 text-center text-slate-400">No hay movimientos registrados</td></tr>
              ) : (
                history.map((log) => (
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <select className="w-full p-3 border rounded-xl outline-none bg-slate-50" value={form.brand_id} onChange={(e) => setForm({ ...form, brand_id: e.target.value })}>
              ...
              <div>
                ...
                <input required type="number" className="w-full p-3 border rounded-xl outline-none" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} placeholder="Ej: 5" />
              </div>
              <div>
                <label className="block text-sm font-bold mb-1">Notas</label>
                <textarea className="w-full p-3 border rounded-xl outline-none" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows="2" placeholder="Proveedor, Nro Factura..."></textarea>
              </div>
              <button type="submit" disabled={submitting} className="w-full bg-[#0056b3] text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-blue-700">
