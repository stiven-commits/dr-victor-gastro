import { Users, LogOut, HeartPulse, ShieldAlert, Calendar, DollarSign, X } from 'lucide-react';
...
export default function Sidebar({ currentUser, activeTab, setActiveTab, handleLogout, mobileMenuOpen, setMobileMenuOpen }) {
  const handleTabClick = (tab) => {
    setActiveTab(tab);
    if (setMobileMenuOpen) setMobileMenuOpen(false); // Cierra el menÃº en mÃ³vil al elegir una opciÃ³n
  };

  return (
    <>
      {/* Capa oscura (Overlay) solo para mÃ³vil */}
      {mobileMenuOpen && (
        <div 
      ...
      {/* Sidebar - Convertido en un cajÃ³n deslizante para mÃ³vil */}
      <aside className={`fixed md:sticky top-0 left-0 h-screen z-50 w-64 bg-white shadow-2xl md:shadow-lg flex flex-col transform transition-transform duration-300 ease-in-out md:translate-x-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-gray-100 flex flex-col items-start relative">
          ...
          <img src={logoBlue} alt="Logo Dr. VÃ­ctor" className="h-10 md:h-12 w-auto object-contain mb-3" />
        ...
        <nav className="p-4 space-y-2 flex-1 overflow-y-auto">
          ...
          <button onClick={() => handleTabClick('patients')} className={`flex items-center w-full gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'patients' ? 'bg-[#0056b3] text-white shadow-md' : 'text-slate-600 hover:bg-blue-50 hover:text-[#0056b3]'}`}>
            <HeartPulse className="w-5 h-5" /> <span className="font-medium">Pacientes ClÃ­nicos</span>
          </button>
          
          {/* BOTÃ“N DE AGENDA OCULTO TEMPORALMENTE
          ...
          */}
          ...
          <button onClick={() => handleTabClick('finances')} className={`flex items-center w-full gap-3 px-4 py-3 rounded-lg transition ${activeTab === 'finances' ? 'bg-[#0056b3] text-white shadow-md' : 'text-slate-600 hover:bg-blue-50 hover:text-[#0056b3]'}`}>
            <DollarSign size={20} /> <span className="font-medium">Finanzas / Pagos</span>
          </button>
